...
unless File.exists?(Rails.root.join('config', 'site.yml'))
  raise "It seems that there is no site.yml config. I need it! Desired location: #{Rails.root.join('config', 'site.yml')}"
end
